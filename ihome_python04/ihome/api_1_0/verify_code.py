...
def get_sms_code(mobile):
    ...
    print("读取到real_image_code.lower() ：",  real_image_code.lower())
    print("读取到image_code.lower() ：", image_code.lower())
    # 与用户填写的值进行对比
    if real_image_code.lower() != image_code.lower():
        # 表示用户填写错误
        return jsonify(errno=RET.DATAERR, errmsg="图片验证码错误")
    ...
    #send_sms(mobile, [sms_code, int(constants.SMS_CODE_REDIS_EXPIRES / 60)], 1)
